...
/datum/visioneffect/cult_conversion/process_hud(var/mob/caster)
	...
	var/turf/T
	T = get_turf(caster)
	for(var/mob/living/carbon/target in (caster.loneliness_affected() ? list(caster) : dview(client_view_num(caster.client.view)+DATAHUD_RANGE_OVERHEAD, T, INVISIBILITY_MAXIMUM)))
